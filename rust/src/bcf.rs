    ...
    I: IntoIterator<Item = Result<RawVcfRecord>>,
{
    ...
    let temp_bcf = temporary_path(path);
...
fn replace_pair(temp_data: &Path, data: &Path, temp_index: &Path, index: &Path) -> Result<()> {
    if data.exists() {
        fs::remove_file(data)?;
    }
    if index.exists() {
    ...
    if let Err(error) = fs::rename(temp_index, index) {

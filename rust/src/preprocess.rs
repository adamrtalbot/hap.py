...
use crate::{fasta, output::OutputTransaction, partial_credit, variant_pipeline, vcf};
...
pub fn run(mut args: PreprocessArgs) -> Result<()> {
    if args.version {
...
mod tests {
    ...
    fn plain_vcf_output_failure_leaves_unindexed_output() -> Result<()> {
        ...
        assert!(
            !output.exists(),
            "a failed generation must not publish its partially written VCF"
        );

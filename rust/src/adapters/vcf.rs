...
impl Variant {
    ...
    pub(crate) fn is_single_base_snp(&self) -> bool {
    ...
    pub(crate) fn active_alts(&self) -> Vec<&str> {
    ...
    pub(crate) fn is_homalt(&self) -> bool {
...
impl RawVcfRecord {
    pub(crate) fn from_line(line: &str, path: &Path) -> Result<Self> {
    ...
    pub(crate) fn effective_end_pos(&self, path: &Path) -> Result<usize> {
        if self.pos == 0 {
    ...
    pub(crate) fn format_keys(&self) -> Vec<&str> {
        self.format
            .as_deref()
    ...
    pub(crate) fn sample_map(&self, index: usize) -> BTreeMap<String, String> {
...
pub(crate) fn canonical_gt(gt: &str) -> String {
    match gt {
        "1" => "1/1".to_string(),
        "0" => "0/0".to_string(),
        "." => "./.".to_string(),
...
pub(crate) fn normalize_chrom(chrom: &str, reference_contigs: &BTreeSet<String>) -> String {
...
pub(crate) fn matches_interval_filters(
    chrom: &str,
    pos: usize,
    effective_end: usize,
    regions: Option<&[Interval]>,
...
pub(crate) fn load_bed(path: &Path, reference_contigs: &BTreeSet<String>) -> Result<Vec<Interval>> {
...
mod tests {
    ...
    fn variant_loading_distinguishes_region_overlap_from_target_start() -> Result<()> {
        ...
        let right_of_span = vec![Interval {
            chrom: "chr1".to_string(),
            start: 5,
            end: 6,
        }];

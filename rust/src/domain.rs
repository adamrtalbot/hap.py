...
pub struct GenomicPosition(NonZeroUsize);
...
impl GenomicCoordinate {
    ...
    pub fn new(contig: impl Into<String>, position: GenomicPosition) -> Result<Self, DomainError> {
...
impl AlleleIndex {
    ...
    pub fn new(value: usize, alternate_allele_count: usize) -> Result<Self, DomainError> {
...
#[derive(Clone, Copy, Debug, Eq, Hash, Ord, PartialEq, PartialOrd)]
pub struct Ploidy(NonZeroUsize);
...
impl Genotype {
    ...
    pub fn parse(value: &str, alternate_allele_count: usize) -> Result<Self, DomainError> {
        if value.is_empty() {
    ...
    #[cfg_attr(not(test), allow(dead_code, reason = "domain accessor"))]
    pub const fn ploidy(&self) -> Ploidy {
        self.ploidy
    }

    /// Returns the genotype phasing state.
    #[cfg_attr(not(test), allow(dead_code, reason = "domain accessor"))]
    pub const fn phase(&self) -> GenotypePhase {
        self.phase
    }
...
#[derive(Clone, Copy, Debug, Eq, Hash, Ord, PartialEq, PartialOrd)]
pub struct QueryRecordIndex(usize);

impl QueryRecordIndex {
    /// Constructs an index checked against the number of query records.
    pub fn new(index: usize, query_record_count: usize) -> Result<Self, DomainError> {
...
impl QueryProvenance {
    ...
    pub fn source(index: usize, query_record_count: usize) -> Result<Self, DomainError> {
    ...
    pub fn parse(value: &str, query_record_count: usize) -> Result<Self, DomainError> {
        if value == "." {
...
impl OutputPlan {
    ...
    pub fn counts_path(&self) -> Option<PathBuf> {
        self.write_counts
    ...
    pub fn conflicts_with_input(&self, input: &Path) -> bool {
